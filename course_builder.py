...
from helpers import is_course_complete, mkdir_p, normalize_path
from constants import IN_MEMORY_MARKER, FORMAT_MAX_LENGTH, TITLE_MAX_LENGTH
...
# ---------------------------------------------------------------------------
# Filename formatting (merged in from the former formatting module)
# ---------------------------------------------------------------------------
...
def get_lecture_filename(combined_section_lectures_nums, section_dir,
                         secnum, lecnum, lecname, title, fmt):
    """
    Prepare a destination lecture filename, trimming overly long
    format/title parts to avoid 'filename too long' errors.
    """
...
# ---------------------------------------------------------------------------
# Resource filtering (merged in from the former filtering module)
# ---------------------------------------------------------------------------

# These formats are trusted and are never skipped.
VALID_FORMATS = r"""^mp4$|
                    ^pdf$|
                    ^.?.?\.?txt$|
                    ^.?.?\.?srt$|
...
# A non-simple format contains characters besides letters, numbers, "_" and "-".
...
def skip_format_url(format_, url):
    """
    Return True if a given format/url should be skipped and not downloaded.
    """
    ...
    # If this is a non simple format, skip it
...
def find_resources_to_get(lecture, file_formats, resource_filter, ignored_formats=None):
    """
    Select which formats of a lecture to download.
    """
    resources_to_get = []
    ...
    for fmt, resources in lecture.items():
        ...
        if fmt in file_formats or (short_fmt is not None and short_fmt in file_formats) or 'all' in file_formats:
            for r in resources:
...
# ---------------------------------------------------------------------------
# M3U playlist (merged in from the former playlist module)
# ---------------------------------------------------------------------------

def create_m3u_playlist(section_dir):
    """
    Create an M3U playlist of every *.mp4 found under `section_dir`.
    The playlist is written into that directory.
    """
